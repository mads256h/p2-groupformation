...
(function(){
    ...
    function createGroupSvg(group){
        ...
        svgDiv.setAttribute("style", "width: " + svgWidth + "px; display: inline-block;");
    ...
    function createSubject(group, subjectName, svg, yValue){
        ...
        // Create the circles
        const arrCircleSize = closeby(getSubjectValuesOfGroup(group, subjectName));
        for (const i in arrCircleSize) {
            const xValue = subjectCircleXValue(group.students[i].criteria.subjectPreference.subjects[subjectName].score);
    ...
    function createLearningStyleDimension(group, learnStyleName, svg, yValue){
        ...
        for (const i in arrCircleSize) {
            const xValue = lSCircleXValue(group.students[i].criteria.learningStyles[learnStyleName]);
            const studentColor = colorArr[i];
            svg.appendChild(createCircle(xValue, yValue, studentColor, arrCircleSize[i]));
        }
    }
     ...
     */
    function createBar(yValue, minimalValue, totalRange, svg, title){
     ...
     */
    function lSCircleXValue(learnStyleValue){
        ...
        const percent = (learnStyleValue + 11) / LSRANGEWIDTH; // from -11 to 11, how many % is the position into the line? 0%=-11, 100%=11
    ...
    /**
     * @summary calculates the x coordinate of the circle's center from the subject score(from 0 to 1) to a x coordinate on the line
     * @param {number} score the subject score form which to calculate the x value of the circle
     * @returns {number} Returns the x coordinate of the circle
     */
    function subjectCircleXValue(score){
        let xPos = svgLineSpace; // add the ofset from the left of the svg (the lines start some length inside the grey) this is equal to the 0 position
        const lineLength = svgWidth - (2 * svgLineSpace); // the length of the line
        xPos += lineLength * score; // how far into the line must the center of the circle be placed
        return xPos;
    }
     ...
     */
    function closeby(groupLSValues){ // something is wrong in this function, but it still kinda works
        ...
        for (const i in groupLSValues) {
            ...
            for (const j in groupLSValues) {
                ...
                if (i !== j && range(groupLSValues[i], groupLSValues[j])){
                    resArr[j]++;
                }
            }
        }
